...
static const char *s_pBattleRoyaleWinnerPropModels[] =
	{
...
static const char *s_pBattleRoyaleWinnerMessages[] =
	{
		"%s's defeat procedure failed.",
		"%s's victory has been confirmed. No causal relationship with skill was found.",
		"%s has been declared the winner by process of elimination.",
		"%s's victory application has been approved.",
		"%s has won. The result is not statistically significant.",
...
class CBattleRoyaleWinnerPropRain : public CBaseEntity
{
public:
	DECLARE_CLASS(
...
CHL2MPRules::CHL2MPRules()
{
	...
	m_tmNextPeriodicThink = 0.0f;
	m_flRestartGameTime = 0.0f;
	...
	m_eBattleRoyaleState = BR_ROUND_WAITING;
	m_flBattleRoyaleStateEndTime = 0.0f;
	m_flBattleRoyaleRoundEndTime = 0.0f;
	m_iBattleRoyaleStartingPlayers = 0;
...
int CHL2MPRules::GetBattleRoyalePlayerCount() const
{
	int playerCount = 0;
...
void CHL2MPRules::FreezeBattleRoyalePlayers(bool bFrozen)
{
...
void CHL2MPRules::EnterBattleRoyaleWaiting()
{
	m_eBattleRoyaleState = BR_ROUND_WAITING;
	m_flBattleRoyaleStateEndTime = 0.0f;
	m_flBattleRoyaleRoundEndTime = 0.0f;
...
void CHL2MPRules::StartBattleRoyaleCountdown()
{
	m_eBattleRoyaleState = BR_ROUND_COUNTDOWN;
	m_flBattleRoyaleStateEndTime =
		gpGlobals->curtime + br_countdown_time.GetFloat();
	...
	Msg(
		"[BR] Round starts in %.0f seconds\n",
		br_countdown_time.GetFloat());
}
...
void CHL2MPRules::StartBattleRoyaleRound()
{
	...
	StartBattleRoyaleZone();

	Msg(
		"[BR] Round started with %d player(s)\n",
		m_iBattleRoyaleStartingPlayers);
}

void CHL2MPRules::FinishBattleRoyaleRound(
	CBasePlayer *winner)
{
	if (m_eBattleRoyaleState == BR_ROUND_FINISHED)
	{
		return;
	}
	...
	m_flBattleRoyaleStateEndTime =
		gpGlobals->curtime +
		br_restart_time.GetFloat();

	FreezeBattleRoyalePlayers(
		true);
	...
	if (winner)
	{
		...
		Q_snprintf(
			message,
			sizeof(message),
			s_pBattleRoyaleWinnerMessages[messageIndex],
		...
		UTIL_ClientPrintAll(
			HUD_PRINTTALK,
			"[SERVER] Winner selection completed successfully.");
	...
	UTIL_ClientPrintAll(
		HUD_PRINTCENTER,
		"Nobody survived. Excellent work.");

	UTIL_ClientPrintAll(
		HUD_PRINTTALK,
		"[SERVER] No acceptable player was found.");

	Msg(
		"[BR] Round ended without "
		"an acceptable player\n");
}

void CHL2MPRules::UpdateBattleRoyaleRound()
{
	...
	case BR_ROUND_WAITING:
	{
		...
		if (playerCount >= br_min_players.GetInt())
		{
			StartBattleRoyaleCountdown();
		}
	...
	case BR_ROUND_COUNTDOWN:
	{
		...
		{
			StartBattleRoyaleRound();
			break;
		}

		const int secondsRemaining =
			static_cast<int>(timeRemaining) + 1;
...
bool CHL2MPRules::FPlayerCanRespawn(
	CBasePlayer *pPlayer)
{
	if (
		m_eBattleRoyaleState == BR_ROUND_ACTIVE ||
		m_eBattleRoyaleState == BR_ROUND_FINISHED)
	{
		return false;
	}
...
bool CHL2MPRules::ShouldPlayerObserveBattleRoyale(
	const CHL2MP_Player *player) const
{
	if (!player)
	{
		return false;
	...
	return !player->IsBattleRoyaleParticipant();
}
